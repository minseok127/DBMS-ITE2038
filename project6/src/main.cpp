...
char log_path[20] = "logfile.data";
char logmsg_path[20] = "logmsg.txt";
...
#define SST_TABLE_SIZE			(10)
#define SST_OPERATION_NUMBER	(10)
...
SST_func(void* args)
{
	...
	for (;;) {
		...
		if (rand() % 2 == 0){
			...
			cout << "update key : " << key2 << "into value : " << value << endl;
			if (ret != 0) {
...
single_thread_test()
{
	...
	int transaction_id = trx_begin();

	char ret_val[120];
	for(int i = 0; i < 10 ; i++){
		db_find(1, i, ret_val, transaction_id);
		cout << "key " << i << " is value " << ret_val << endl;
	}
...
mlock_test()
{
	...
	/* Initiate database. */
	init_db(3, 0, 0, log_path, logmsg_path);
...
deadlock_test()
{
	...
	/* Initiate database. */
	init_db(DATABASE_BUFFER_SIZE, 2, 5, log_path, logmsg_path);
	exit(1);
	printf("database init\n");
