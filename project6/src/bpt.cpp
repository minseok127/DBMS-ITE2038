...
pthread_mutex_t tree_latch = PTHREAD_MUTEX_INITIALIZER;
...
int init_db(int num_buf, int flag, int log_num, char* log_path, char* logmsg_path){
	...
	/* Initialize buffer instances for buffer manager */
	LRU_Head = new Buffer;
	LRU_Tail = new Buffer;
	...
	if(init_trxManager() == -1){
		return -1;
	}

	/* Initailize log manager */
	log_manager = new LogManager(log_path);
	if (log_manager == NULL){
		return -1;
	}
	MetaLog* metaLog = log_manager->get_metaLog();

	/* Initailize recovery manager */
	recovery_manager = new RecoveryManager(log_manager->get_logFileFd(), logmsg_path, metaLog);
	...
	/********************** Recovery test **********************/
	/* Analysis pass */
	recovery_manager->analysis();

	/* Redo, Undo */
	switch (flag)
	{
	case NORMAL_RECOVERY:
		recovery_manager->redo(NORMAL_RECOVERY);
		recovery_manager->undo(NORMAL_RECOVERY);
		break;
	case REDO_CRASH:
		recovery_manager->redo(log_num);
		break;
	case UNDO_CRASH:
		recovery_manager->redo(NORMAL_RECOVERY);
		recovery_manager->undo(log_num);
		break;
	}
	
	/* Log flush */
	log_manager->bufFlush();
	/* Close target files and flush dirty pages */
	stack<int> idStack = recovery_manager->get_table_id_stack();
	while(!idStack.empty()){
		int table_id = idStack.top();
		idStack.pop();
	...
	/* If recovery success */
	if (flag == NORMAL_RECOVERY){
		/* Remain last published LSN and last trx id for new log publishing and make new trx */
		metaLog->metaLSN = log_manager->get_tailBuf();
		metaLog->metaTrxId = metaLog->metaTrxId > recovery_manager->get_maxTrxId() ? metaLog->metaTrxId : recovery_manager->get_maxTrxId();
		int fd = log_manager->get_logFileFd();
		
		lseek(fd, 0, SEEK_SET);
		write(fd, metaLog, sizeof(MetaLog));

		/* Truncate log file. Only remains meta log */
		ftruncate(fd, sizeof(MetaLog));
		/* Set next trx id */
		trx_manager->set_next_trx_id(metaLog->metaTrxId + 1);
	}
...
int open_table(char* path) {
	/* path is not started "DATA" or 
	 * after "DATA", there is no number or
	 * path is (DATA**), buf ** is bigger than 10 */
	if (strncmp("DATA", path, 4) != 0 || atoi(path + 4) == 0 || atoi(path + 4) > 10){
		return -1;
	}

	int table_id = atoi(path + 4);
...
int db_find(int table_id, int64_t key, char* ret_val, int trx_id) {
	...
	/* Get header page's information*/
	buffer_read_page(table_id, 0, &headerPage);
	buffer_complete_read_without_write(table_id, 0);

	/* Take tree mutex while finding leaf page */
	pthread_mutex_lock(&tree_latch);

	/* Start from root page */
	buffer_read_page(table_id, headerPage.root_pageNum, &internalPage);

	leafPageNum = headerPage.root_pageNum;
	prev = leafPageNum;
	while(!internalPage.isLeaf){
		/* Find area where key can be exists */
		i = search_index_area_internal(internalPage.node, key, internalPage.num_keys);
		if (i == -1){
		...
		buffer_read_page(table_id, leafPageNum, &internalPage);
		buffer_complete_read_without_write(table_id, prev);
		prev = leafPageNum;
	}
	pthread_mutex_unlock(&tree_latch);

	/* Copy bytes from internal page instance to leaf page instance */
	memcpy(&target_leafPage, &internalPage, PAGE_SIZE);

	/* Find key's location */
	i = search_index_location_leaf(target_leafPage.record, key, target_leafPage.num_keys);
	if (i == -1){
		/* If key does not exist, db_find is failed. */ 
		buffer_complete_read_without_write(table_id, leafPageNum);
		trx_abort(trx_id);
		return -1;
	}

	/* This will be used lock_wait() */
	lock_t* ret_lock;

	/* Try to acquire lock */
	switch(lock_acquire(table_id, key, trx_id, S, &ret_lock))
	{
		/* If succeed to acquire the record lock witout wating, release the lock manager latch and go ahead */
		/* lock manager latch is released in lock_acquire() when this trx do not have to wait */
		case ACQUIRED:
			/* page latch & record lock are acquired */
			strcpy(ret_val, target_leafPage.record[i].value);
			buffer_complete_read_without_write(table_id, leafPageNum);

			return 0;
		case NEED_TO_WAIT:
			/* page latch & transaction latch is acquired */
			...
			/* Find key's page from root page again. */
			pthread_mutex_lock(&tree_latch);
			buffer_read_page(table_id, headerPage.root_pageNum, &internalPage);
			...
			/* lock_wait() is done => lock acquired => get the page again */
			/* Do not release the page latch. Because lock acquired. */
			memcpy(ret_val, target_leafPage.record[i].value, 120);
			buffer_complete_read_without_write(table_id, leafPageNum);

			return 0;
		case DEADLOCK:
			/* Only page latch is acquired. Release it */
			buffer_complete_read_without_write(table_id, leafPageNum);
			trx_abort(trx_id);
			return -1;
	}
...
int db_update(int table_id, int64_t key, char* values, int trx_id){
	...
	/* Finding start from root page */
	buffer_read_page(table_id, headerPage.root_pageNum, &internalPage);

	leafPageNum = headerPage.root_pageNum;
	prev = leafPageNum;
	while(!internalPage.isLeaf){
		/* Find area where key can be exist */
		i = search_index_area_internal(internalPage.node, key, internalPage.num_keys);
		if (i == -1){
			leafPageNum = internalPage.farLeft_pageNum;
		}
	...
	/* Copy bytes of internal page instance to leaf page instance */
	memcpy(&target_leafPage, &internalPage, PAGE_SIZE);
	...
	/* This will be used lock_wait() */
	lock_t* ret_lock;
	/* pageLSN for update page information */
	uint64_t pageLSN;
	/* offset for publish new log */
	int offset;

	switch(lock_acquire(table_id, key, trx_id, X, &ret_lock))
	{
		/* If succeed to acquire the record lock witout wating, release the lock manager latch and go ahead */
		/* lock manager latch is released in lock_acquire() when this trx do not have to wait */
		case ACQUIRED:
			/* page latch & record lock are acquired */
			/* PAGE_HEADER_SIZE = 128, Record = key(int64_t, 8byte) + value(char[120], 120byte), key is int64_t */
			/* offset is values physical location */
			offset = PAGE_HEADER_SIZE + (i * sizeof(Record)) + sizeof(int64_t);
			/* Before update, publish update log */
			/* undo is original value = target_leafPage.record[i].value, redo is updated value = values */
			/* publish_updatelog(trx id, table id, pagenum, offset, undo, redo) */
			//pthread_mutex_lock(&log_manager_latch);
			pageLSN = log_manager->publish_updateLog(trx_id, table_id, leafPageNum, offset, target_leafPage.record[i].value, values);
			//pthread_mutex_unlock(&log_manager_latch);

			/* store_original_log(trxNode, table_id, key, redo, undo) */
			trx_manager->store_original_log(targetTrx, table_id, key, leafPageNum, values, target_leafPage.record[i].value);

			/* Update */
			memcpy(target_leafPage.record[i].value, values, 120);
		...
		case NEED_TO_WAIT:
			...
			/* Find leaf page from root page again. */ 
			pthread_mutex_lock(&tree_latch);
			...
			/* undo is original value = target_leafPage.record[i].value, redo is updated value = values */
			/* publish_updatelog(trx id, table id, pagenum, offset, undo, redo) */
			/* Log manager's latch will be acquired in publish function */
			pageLSN = log_manager->publish_updateLog(trx_id, table_id, leafPageNum, offset, target_leafPage.record[i].value, values);
...
int db_insert(int table_id, int64_t key, char* value){
	...
	int i = search_index_location_leaf(leafPage.record, key, leafPage.num_keys);
	if (i != -1) {
		buffer_complete_read_without_write(table_id, leafPageNum);
		return -1;
	}
...
int db_delete(int table_id, int64_t key){
	...
	delete_entry(table_id, headerPage.root_pageNum, leafPageNum, key);
	return 0;
	/*
...
treeNode dequeue(void){
    treeNode n = *treeQueue;
	treeNode* tmp = treeQueue;
    treeQueue = treeQueue->next;
	delete tmp;
    n.next = NULL;
    return n;
}
