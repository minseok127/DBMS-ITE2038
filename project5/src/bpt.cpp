...
int db_find(int table_id, int64_t key, char* ret_val, int trx_id) {
	...
	/* Get information of header page */
	buffer_read_page(table_id, 0, &headerPage);
	buffer_complete_read_without_write(table_id, 0);
	...
	switch(lock_acquire(table_id, key, trx_id, S, &ret_lock))
	{
		...
		case NEED_TO_WAIT:
			...
			/* Get the information of header page again */
			buffer_read_page(table_id, 0, &headerPage);
			buffer_complete_read_without_write(table_id, 0);

			/* Find again the leaf page where key should be */
			leafPageNum = find_leafPage(table_id, headerPage.root_pageNum, key);

			/* Do not release the page latch. Because lock acquired. */
			buffer_read_page(table_id, leafPageNum, &target_leafPage);

			/* Find key's location again */
...
int db_update(int table_id, int64_t key, char* values, int trx_id){
	...
	switch(lock_acquire(table_id, key, trx_id, X, &ret_lock))
	{
		...
		case ACQUIRED:
			...
			trx_manager->store_original_log(targetTrx, table_id, key, values, target_leafPage.record[i].value);
